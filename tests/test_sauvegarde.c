...
int main(){
    ...
    printf("Sauvegarde avec joueur=1 et (x,y) dernier coup=(3,4) avec la var (=1)\n\n");
    save("test_save1.txt",grille,morpion,1,3,4,1);
    ...
    afficher_morpion(morpion);
    int joueur,xdc,ydc,var;
    printf("Initialistion variables de jeu, joueur = %i, coordonnées dernier coup = (%i, %i) et var = %i\n",joueur,xdc,ydc,var);
    printf("Chargement avec fichier test_save2.txt\n");
    slot = fopen("test_save2.txt","r");
    fscanf(slot,"%s\n",savedata);
    printf("Données de sauvegarde 2 = %s\n",savedata);
    load("test_save2.txt",grille,morpion,&joueur,&xdc,&ydc,&var);
    printf("Nouvelles variables de jeu, joueur = %i, coordonnées dernier coup = (%i, %i) et var = %i\n",joueur,xdc,ydc,var);
