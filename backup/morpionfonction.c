...
int morpiongagne(int grille[M][M]){
    ...
    return 0; 
    /**
    \return la valeur du vainqueur sinon 0.
    */
}
/**
...
/**
\fn int debut_de_tour_term(int xdc, int ydc, int joueur, int morpion[M][M])
\brief Effectue les affichages du début de tour et vérifie où le joueur peut jouer.
\brief Prend en paramètre le joueur et les coordonnées du dernier coup joué, ainsi que le morpion.
\return le numéro de la case (0-8) où le joueur peut jouer, 9 si partout.
*/
...
/**
\fn void tour_du_joueur_term(int carre_jouable, int * joueur, int grille[N][N], int * xdc, int * ydc)
\brief Fonction qui demande le coup au joueur et vérifie qu'il soit valide, et prend en paramètre le carré jouable, le joueur dont c'est le tour, la grille et l'adresse du dernier coup joué.
*/
...
/**
\fn void morpion_term (int reload, option_t opt)
\brief Fonction qui lance la partie de morpion et qui prends en paramètre VRAI si une partie sauvegardée doit ếtre chargée, FAUX sinon. Ainsi que les options de jeu.
*/
void morpion_term (int reload, option_t opt){
    ...
    while (! vainqueur){ ///Tant qu'il n'y a pas de vainqueur la partie continue.
