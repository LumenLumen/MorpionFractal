...
/**
\fn valideCase (int * joueur, int grille[N][N], int morpion[M][M], int * xdc, int * ydc, int x, int y)
\brief Fonction qui prends en paramètre le joueur dont c'est le tour, la grille 9*9, le morpion 3*3, les coordonnées de la dernière case jouée, et les coordonnées de la case jouée.
*/
...
int valideCase (int * joueur, int grille[N][N], int morpion[M][M], int * xdc, int * ydc, int x, int y){
    ...
    if((coog_to_carr(x,y) != carre) && (carre != 9)){
        ...
        *ydc = y ;//Les coordonnées sont valides et deviennent le nouveau dernier coup joué.
        grille[x][y] = *joueur ; ///On ajoute la valeur valide du joueur à la grille à l'endroit joué.
