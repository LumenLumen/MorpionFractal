...
int gamescreen (SDL_Window * window, SDL_Renderer * renderer, int reload){
    ...
    if (reload){ ///si reload=1, on charge la partie sauvegardée sinon, on initialise une nouvelle grille.
        ...
        while(SDL_PollEvent(&event)){ 
            ...
            switch(event.type){
                ...
                case SDL_MOUSEBUTTONDOWN :
                    if (SDL_ClickInButton(event.button.y, event.button.x, contour)){ ///Lorsque le joueur clique dans la grille, on vérifie dans quelle case il a joué pour s'assurer que son coup est valide.
                    ...
                    if (SDL_ClickInButton(event.button.x, event.button.y, sauv_rec)){ ///Si le joueur clique sur le bouton de sauvegarde, sa partie sera sauvegardée.
    ...
    /// lorsqu'un des deux joueur remporte la partie on annonce le vainqueur, puis on revient à la page d'accueil.
...
int menuscreen (void){
   ...
   SDL_RenderCopy(renderer,texte_tex,NULL,&newgame);
    ...
    ///On libère la mémoire si l'on clique sur le bouton "Quitter"
    SDL_DestroyRenderer(renderer);
    SDL_DestroyWindow(window);
    TTF_Quit();
    SDL_Quit();
